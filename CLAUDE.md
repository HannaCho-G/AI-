...
Claude Code를 지휘자로 삼아 전자책을 기획→조사→집필→검증→편집→비주얼→출판→교차판매 연결까지 파이프라인으로 생산하는 프로젝트.
...
| 8 | Content Connector | `.claude/agents/content-connector.md` | 기존 책들과의 교차판매/연관 콘텐츠 매핑 |
...
너 (아이디어 + 승인)
 ↓
Publisher → Researcher → Writer → Fact Checker → Editor → Visual Director → Production → Content Connector
 ↓
완성된 책
```
...
07-cross-sell.md          # Content Connector의 연관 콘텐츠 맵
```
