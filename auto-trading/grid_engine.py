...
def main():
    ...
    if args.confirm_withdrawal is not None:
        ...
        #
        # 주의: amount를 기준선에서 단순히 빼는 게 아니라, "출금 후 지금 계좌에
        # 남아있는 실제 평가자산"을 다시 조회해서 그 값을 새 기준선으로 잡는다
        # (confirm_withdrawal() 참고). amount는 "얼마를 출금했다고 확정할지"와
        # 누적출금 합계 기록용이지, 기준선 계산식의 뺄셈 항이 아니다.
        print(f"⚠️  ₩{amount:,.0f}을 업비트 앱에서 실제로 출금하셨다는 전제로, "
              f"출금 후 지금 남은 실제 평가자산을 다시 조회해서 새 기준선으로 잡습니다. "
              f"이 프로그램은 실제 출금 여부를 확인할 수 없습니다.")
        try:
